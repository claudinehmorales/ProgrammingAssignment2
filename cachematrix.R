...
makeCacheMatrix <- function(x = matrix()) { 
        ...
        ## set():  assigns the matrix passed in makeCacheMatrix()
        ...
        ##  get():  returns the matrix passed in makeCacheMatrix():
        get <- function() x
...
cacheSolve <- function(x) {
        ...
        ## check if inverse of your matrix has been cached:
        ...
        if(!is.null(invrs)){
              ...
              ## If it has NOT been stored, the value of your matrix is assigned to the variable newMatrix: 
              newMatrix <- x$get()
          
              ## Its inverse is then calculated:
              invrs <- solve(newMatrix)
          
              ## And then stored (so that if you want to get it again, no new expensive calculation will be made!):
